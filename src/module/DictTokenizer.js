...
exports.split = function (words) {
  ...
  for (var i = 0, word; word = words[i]; i++) {
    ...
    // 仅对未识别的词进行匹配
    const wordinfo = matchWord(word.w, 0, words[i - 1]);
...
var matchWord = function (text, cur, preword) {
  ...
  const ret = [];
  const s = false;
  const TABLE = exports.segment.getDict('TABLE2');
  // 匹配可能出现的单词
...
var filterWord = function (words, preword, text) {
  ...
  // 将单词按位置分组
  const wordpos = getPosInfo(words, text);
  // debug(wordpos);
  ...
  // 取以上几项综合排名最最好的
  const chunks = getChunks(wordpos, 0, text);
  // debug(chunks);
  const assess = []; // 评价表
  ...
  for (var i = 0, chunk; chunk = chunks[i]; i++) {
    assess[i] = {
      x: chunk.length, a: 0, b: 0, c: 0, d: 0,
    };
    // 词平均长度
...
var getChunks = function (wordpos, pos, text) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (!wordpos[nextcur]) {
      ...
      const chunks = getChunks(wordpos, nextcur);
...
var getTops = function (assess) {
  ...
  // 取各项最大值
  const top = {
    x: assess[0].x, a: assess[0].a, b: assess[0].b, c: assess[0].c, d: assess[0].d,
  };
